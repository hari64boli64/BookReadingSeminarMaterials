...
\title{Supplemental Material for 07/06\\Book Reading Seminar}
...
\myBox{関数の近傍の定義}{
    ...
    \]
    を $S$ 上の一様な $\varepsilon$-近傍とする。$\norm{G_k-H}_S \coloneqq \sup_{y \in S} \norm{G_k(y)-H(y)} \to 0$ を $G_k$ が $S$ 上で $H$ に収束することと呼ぶ。(一様収束に近い定義)
}

\bookBox{命題5.1.2:近傍解は孤立解に収束する}{
    ...
    しかも $x^\infty\in\overline N$ なので 式(5.1.1) より $x^\infty=x^*$ である。
...
\myBox{degree theory}{
    ...
    \]
    は、大雑把には $\Omega$ 内にある方程式 $\Phi(x)=p$ の解の向き付き個数を表す。
...
\bookBox{命題5.1.3:natural mapによる局所可解性}{
    $F_K^{\mathrm{nat}}$ をnatural mapとする。孤立解 $x^*$ における \red{$\operatorname{ind}(F_K^{\mathrm{nat}},x^*)$ が非零}ならば、式(5.1.1) を満たす任意の開近傍 $N$ に対し、
    \[
...
\myBox{議論の流れの整理}{
    ...
    \begin{itemize}
        ...
        \item 命題5.1.3 → indの値が非零かどうかが、摂動問題でも解が非空かどうかの本質
    \end{itemize}
    ということが分かった。
...
\bookBox{系5.1.5:パラメータ付きVIの局所可解性と収束 \red{($H$ は $F$ の誤植!)}}{
    ...
    後半も、解があることが既に分かっているので、命題5.1.2 (近傍解は孤立解に収束する) より、式(5.1.3)が従う。
}
...
\myBox{具体例による確認}{
    ...
    \begin{figure}[H]
        \centering
        \fbox{\includegraphics[width=\columnwidth]{prop2.1.4.png}}
    \end{figure}
...
\myBox{議論の流れの整理}{
    ...
    \begin{itemize}
        \item 命題5.1.2 → 摂動問題でも解が非空かどうかが、感度分析の本質
        \item 系5.1.5 → indの値が非零かどうかが、摂動問題でも解が非空かどうかの本質
    \end{itemize}
    ということが分かった。そして、
    \begin{itemize}
...
\myBox{臨界錐}{
    \begin{figure}[H]
        \centering
        \fbox{\includegraphics[width=\columnwidth]{critical_cone.png}}
    \end{figure}
}
...
\bookBox{命題5.1.6:臨界錐上の正値性}{
    ...
    \[
        v^\top F'(x^*;v)>0
        \qquad(0\ne v\in \mathcal{C}(x^*;K,F))                         \tag{5.1.4}
    \]
...
\bookBox{補題5.1.7:解の移動量の制御}{
    式(5.1.4)を満たすのならば、
    $x^*$ 以外に解が存在しない $x^*$ の開近傍 $N$ に対してある $c>0$ が存在し、任意の $q \in \mathbb{R}^n$ に対して、
    \[
...
\myBox{半安定だが安定でない点とは?}{
    半安定性は「摂動後の零点が存在するなら、その零点は元の零点の近くにある」ことを保証するが、
    摂動後の零点の存在自体は要求しない。この違いは次の例で明確になる。

    $H\colon\mathbb R\to\mathbb R$, $H(x)=|x|$ を考える。$0$ は $H$ の孤立零点であり、
    任意の摂動 $G$ とその零点 $x'\in G^{-1}(0)$ に対して
    \[
        |x'-0|=|H(x')|
    \]
    が成り立つ。したがって、定義5.2.3の誤差評価は $c=1$ で成立し、$0$ は半安定である。
    一方、任意に小さい $\varepsilon>0$ に対して
    \[
        G_\varepsilon(x)=|x|+\varepsilon
    \]
    は $H$ の一様な微小摂動だが零点をもたない。ゆえに $0$ は安定ではない。
    B微分 $H'(0;v)=|v|$ の零点は $v=0$ だけなので半安定性の十分条件を満たす一方、
    その指数は $0$ であり、安定性を保証する非零指数条件は満たさない、とも解釈できる。

    これを積分すると、$H$ を勾配にもつ $C^1$ 関数として
    \begin{equation*}
        f(x)=
        \begin{dcases}
            -\frac{1}{2}x^2 & \text{if }x<0, \\
            \frac{1}{2}x^2  & \text{if }x\geq0
        \end{dcases}
    \end{equation*}
    ...
    \begin{figure}[H]
        ...
        \begin{minipage}{0.47\columnwidth}
            ...
            \begin{tikzpicture}[scale=1.15]
                ...
                \draw[->] (0,-2.3) -- (0,2.3) node[above] {$f(x)$};
                \draw[cA,thick,domain=-2:0,samples=80]
                    plot (\x,{-0.5*\x*\x});
                \draw[cA,thick,domain=0:2,samples=80]
                    plot (\x,{0.5*\x*\x});
                \fill[cA] (0,0) circle (1.5pt);
                \node[cA,above left] at (-1.35,-0.9) {$-x^2/2$};
                \node[cA,below right] at (1.25,0.8) {$x^2/2$};
            \end{tikzpicture}
        \end{minipage}
        \hfill
        \begin{minipage}{0.47\columnwidth}
            ...
            \begin{tikzpicture}[scale=1.15]
                \draw[->] (-2.3,0) -- (2.3,0) node[right] {$x$};
                \draw[->] (0,-0.35) -- (0,2.5) node[above] {関数値};
                \draw[cA,thick] (-2,2) -- (0,0) -- (2,2);
                \draw[cD,thick,dashed] (-2,2.45) -- (0,0.45) -- (2,2.45);
                \fill[cA] (0,0) circle (1.5pt);
                \node[cA,above left] at (-1.0,0.4) {$H(x)=|x|$};
                \node[cD,above right] at (-1.2,1.6)
                    {$G_\varepsilon(x)=|x|+\varepsilon$};
                \draw[<->,gray] (0.15,0) -- (0.15,0.45)
                    node[midway,right] {$\varepsilon$};
            \end{tikzpicture}
        \end{minipage}
        \caption{左:$H=\nabla f$ を満たすポテンシャル。右:半安定な零点と、その零点を消滅させる一様摂動。}
    \end{figure}

    なお、$f(x)=x^3/3$ の勾配 $H(x)=x^2$ も、摂動 $x^2+\varepsilon$ によって零点が消える例ではある。
    しかし $|x|\leq c|H(x)|=cx^2$ は $0$ の近傍で成立しないため、この零点は半安定ですらない。
    また、非退化な鞍点ではヘッセ行列が正則で、勾配写像の零点の指数が非零となるため安定である。
}
