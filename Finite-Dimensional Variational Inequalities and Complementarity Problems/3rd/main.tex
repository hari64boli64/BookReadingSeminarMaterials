...
\myBox{natural mapとnormal map}{\\
    ...
    \end{equation*}
    雑理解: VIが $-F$ の $K$ 上安定点を求める問題で、natural mapは $-F$ 方向に進む射影勾配降下法の1ステップ。Prop 1.5.8の同値性も自然、証明はVIと $\Pi$ の定義より不等式評価。\\
...
\bookBox{命題5.1.3:natural mapによる局所可解性}{
    $F_K^{\mathrm{nat}}$ をnatural mapとする。孤立解 $x^*$ における $\operatorname{ind}(F_K^{\mathrm{nat}},x^*)$ が非零ならば、(5.1.1) を満たす任意の開近傍 $N$ に対し、
    \[
...
\bookBox{命題5.1.4:normal mapによる誤差条件の局所化}{
    ...
    $\operatorname{ind}(F_K^{\mathrm{nor}},z^*)$ が非零ならば、(5.1.1) を満たす任意の開近傍 $N$ に対し、
